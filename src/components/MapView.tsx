...
const MapView = ({ allFields, selectedFields, activeField, flyToField, onFlyToDone, onFieldClickOnMap, onAddField, editBoundaryFieldId, onUpdateField, onCancelEditBoundary }: MapViewProps) => {
  ...
  useEffect(() => {
    ...
    console.log("[AgriSense Draw] Drawing mode started");
    const handleClick = (e: mapboxgl.MapMouseEvent) => {
      const vertex: [number, number] = [e.lngLat.lng, e.lngLat.lat];
      console.log("[AgriSense Draw] Vertex added:", vertex);
      setDrawVertices((prev) => {
        const updated = [...prev, vertex];
        console.log("[AgriSense Draw] Total vertices:", updated.length);
        return updated;
      });
    };
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Escape") {
        console.log("[AgriSense Draw] Drawing cancelled (Escape)");
        setDrawMode(false); setDrawVertices([]);
      }
      if (e.key === "Backspace") {
        e.preventDefault();
        console.log("[AgriSense Draw] Undo last vertex (Backspace)");
      ...
      if (e.key === "Enter") {
        setDrawVertices((prev) => {
          if (prev.length >= 3) {
            ...
            console.log("[AgriSense Draw] Polygon created:", geometry);
            setDrawMode(false);
            setShowNewFieldDialog(true);
          } else {
            console.warn("[AgriSense Draw] Need at least 3 vertices, have:", prev.length);
          }
          return prev;
        });
      }
  ...
  const handleSaveNewField = (fieldData: { name: string; crop: string; cropEmoji: string; area: number; color: string; location: string; group?: string; coordinates: [number, number][][]; }) => {
    ...
    console.log("[AgriSense Draw] Field saved:", { id: newField.id, name: newField.name, geometry: { type: "Polygon", coordinates: newField.coordinates } });
    onAddField(newField);
